...
func Init(db *badger.DB, id uint64, gid uint32, log timber.Logger) *DiskStorage {
	w := &DiskStorage{
		db:    db,
		id:    id,
		gid:   gid,
	...
	if prev, err := RaftId(db); err != nil || prev != id {
		if err := w.StoreRaftId(id); err != nil {
			panic(err)
		}
	}
...
func (w *DiskStorage) parseIndex(key []byte) uint64 {
	if len(key) != 20 {
		panic("key is not correct length")
	}
...
func (w *DiskStorage) UpdateCheckpoint(snap *wire.Snapshot) error {
	return w.db.Update(func(txn *badger.Txn) error {
		data := snap.Encode()
...
func (w *DiskStorage) FirstIndex() (uint64, error) {
	...
	if err == nil {
		w.log.Verbosef("setting first index: %d", index+1)
		w.cache.Store(firstKey, index+1)
	} else {
		w.log.Verbosef("error while seekEntry: %v", err)
	}
	return index + 1, err
}
...
func (w *DiskStorage) allEntries(lo, hi, maxSize uint64) (es []raftpb.Entry, rerr error) {
	err := w.db.View(func(txn *badger.Txn) error {
		...
		for itr.Seek(start); itr.Valid(); itr.Next() {
			...
			// below.
			if !(e.Index > lastIndex && e.Index >= lo) {
				panic("shits broke")
			}
			lastIndex = e.Index
...
func (w *DiskStorage) CreateSnapshot(i uint64, cs *raftpb.ConfState, data []byte) error {
	w.log.Verbosef("creating snapshot i=%d, cs %+v", i, cs)
	first, err := w.FirstIndex()
	if err != nil {
		return err
	}
	if i < first {
		w.log.Errorf("i=%d<first=%d, ErrSnapOutOfDate", i, first)
